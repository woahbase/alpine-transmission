...
# runtime flags
MOUNTFLAGS := \
	# -v $(CURDIR)/data/config:/var/lib/transmission/config \
	# -v $(CURDIR)/data/downloads:/var/lib/transmission/downloads \
	# -v $(CURDIR)/data/torrents:/var/lib/transmission/torrents \
...
$(shell docker run --rm --pull=never \
	...
	--version \
	2>&1 \
	| grep 'transmission-daemon' |awk '{ print $$2  }')
endef
# -- }}}
